...
namespace Config
{
	...
	$routes -> add('quemsomos', 'Home::quem_somos');
	$routes -> add('localizacao', 'Home::localizacao');

	/** Ajax **/
	$routes -> add('cidades', 'Ajax::busca_cidades');
	$routes -> add('bairros', 'Ajax::busca_bairros');
	$routes -> add('faqs', 'Ajax::faq_details');
	$routes -> add('faqs/(:num)', 'Ajax::faq_details');
	$routes -> add('comunicados', 'Ajax::comunicados');
	$routes -> add('comunicados/(:num)', 'Ajax::comunicados');

	/**
	 * Verifica o ambiente se a constante ENVIRONMENT for igual a
	 * "production",
	 * não exibirá nenhuma informação na tela para o usuário.
	 * @return $debug   true|false
	 */
	$routes -> add('env', 'Ajax::verifica_ambiente');
