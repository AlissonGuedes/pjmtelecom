...
namespace App\Models
{
	...
	class EmpresaModel extends AppModel {
		 ...
		 * @var array
		 */
		protected $formValidation = 'App\Validations\EmpresaValidation';
		 ...
		 * @return array Model()
		 */
		public function getEmpresa($where = false, $fields = '*')
		{
			...
			// From
			$this -> from('tb_empresa', TRUE);

			/*
			 * Adicionar outras condições...
			 */

			$this -> join('tb_acl_grupo', 'tb_acl_grupo.id = tb_empresa.id_grupo', 'left');
		 ...
		 * @return query
		 */
		public function getCidadeEmpresa($where = false, $fields = '*')
		{
			...
			$this -> from('tb_empresa_cidade U', true);
			$this -> distinct(true);
			$this -> select($fields);
			$this -> join('tb_cidade C', 'C.id = U.id_cidade', 'left');
			$this -> where('U.id_empresa', $_SESSION[USERDATA]['id']);
		 ...
		 * @return query
		 */
		public function getBairroEmpresa($where = false, $fields = '*')
		{
		...
		public function login()
		{
			...
			$user = $this -> getEmpresa() -> orWhere($login) -> getRowArray();
